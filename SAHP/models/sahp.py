...
class SAHP(nn.Module):
    ...
    def __init__(self, nLayers, d_model, atten_heads, dropout, process_dim, device, max_sequence_length, rho, lambda_, alpha, n_it, mode):
        ...
        self.attention = MultiHeadedAttention(h=atten_heads, d_model=self.d_model, rho = rho, lambda_=lambda_, alpha=alpha,n_it=n_it, mode='softmax')
        self.attentionB = MultiHeadedAttention(h=atten_heads, d_model=self.d_model, rho = rho, lambda_=lambda_, alpha=alpha,n_it=n_it, mode=mode)
        ...
        self.norm = LayerNorm(size=self.d_model)  #######
    ...
    def forward(self, seq_dt, seq_types, src_mask):
        ...
        for i in range(self.nLayers):
            ...
            # x = self.dropout(self.output_sublayer(x, self.feed_forward))
            if i == self.nLayers-1:
                y, attn = self.attentionB.forward(xx, xx, xx, mask=src_mask)
                x = self.input_sublayer(x, lambda _x: self.attentionB.forward(_x, _x, _x, mask=src_mask))
                x = self.dropout(self.output_sublayer(x, self.feed_forward))
            else:
                y, attn = self.attention.forward(xx, xx, xx, mask=src_mask)
                x = self.input_sublayer(x, lambda _x: self.attention.forward(_x, _x, _x, mask=src_mask))
                x = self.dropout(self.output_sublayer(x, self.feed_forward))
            matrix = attn.cpu().detach().numpy() 
