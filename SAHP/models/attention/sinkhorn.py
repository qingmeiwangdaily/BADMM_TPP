...
class SinkhornDistance(nn.Module):
    ...
    def forward(self, c):
        ...
        x_points = C.shape[-2]   
        y_points = C.shape[-1]   
        batch_size = C.shape[0]  # batch_size*n_heads

        # both marginals are fixed with equal weights
        mu = torch.empty(batch_size, x_points, dtype=torch.float,
                         requires_grad=False, device=C.device).fill_(1.0 / x_points).squeeze()  # batch_size * L
        nu = torch.empty(batch_size, y_points, dtype=torch.float,
                         requires_grad=False, device=C.device).fill_(1.0 / y_points).squeeze()  # batch_size * L
